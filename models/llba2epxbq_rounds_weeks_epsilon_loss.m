function [l, dl, dsurr] = llba2epxbq_rounds_weeks_epsilon_loss(x, D, mu, nui, doprior, options)
    dodiff = nargout == 2;
    np = length(x);
    beta = exp(x(1)); % sensitivity to reward and loss (combined)
    alfa = 1./(1+exp(-x(2))); % learning rate
    epsilon = [exp(x(3)), 0]; % initialize only epsilon2
        ...
        for j = 1:length(unique_sess)
            ...
            for t = 1:length(a)
                ...
                if dodiff
                    ...
                    if rho == 0
                       dl(8) = dl(8) + g * epsilon(2) * (p0(a(t)) * V(s(t)) * ((a(t) == 1) - p0(1))) / pg(a(t)) * (w - 1);
                    end
                 end
